...
【厳守】実在しないジム・店舗名・ダミーURL（example.com等）を絶対に出力しない。
競合を例示する場合は、公式サイトで実在を確認できるジムのみを、事実の範囲で中立に記載する。
確認できない場合は固有名を出さず「〇〇タイプのジム」と一般化する。
自社として挙げてよいのは 5toolgym 薬院店・赤坂大濠公園店 の2店舗のみ。` : '';
